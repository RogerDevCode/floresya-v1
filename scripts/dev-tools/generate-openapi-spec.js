...
async function generateOpenApiSpec() {
  try {
    ...
    // Save as JSON
    const jsonPath = path.resolve(__dirname, '../../api/docs/openapi-spec.json')
    await fs.writeFile(jsonPath, JSON.stringify(swaggerSpec, null, 2))
    console.log(`✅ OpenAPI JSON specification saved to: ${jsonPath}`)

    // Save as YAML
    const yamlPath = path.resolve(__dirname, '../../api/docs/openapi-spec.yaml')
    ...
    const summaryPath = path.resolve(__dirname, '../../api/docs/generation-summary.json')
