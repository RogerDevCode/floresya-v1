...
async function generateApiClient() {
  try {
    ...
    const typesPath = path.resolve(__dirname, '../../public/js/shared/api-types.js')
    ...
    const guidePath = path.resolve(__dirname, '../../public/js/shared/API_USAGE.md')
...
function generateMethodName(path, method, operation) {
  ...
  if (path.match(/\/api\/products\/\{[^}]+\}\/occasions/)) {
    // Specific handler for product occasions (e.g., /api/products/{id}/occasions)
    const actionMap = {
      get: 'getProductOccasions',
      post: 'createProductOccasion',
      put: 'updateProductOccasions',
      patch: 'updateProductOccasions',
      delete: 'deleteProductOccasion'
    }
    return actionMap[method] || `${method}ProductOccasions`
  }
