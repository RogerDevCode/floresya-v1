...
export class PaymentMethodRepository extends BaseRepository {
   ...
   */
  async getStats() {
    const { data, error } = await this.supabase.from(this.table).select('type, is_active')
    ...
    const stats = {
      total: data.length,
      active: data.filter(pm => pm.is_active).length,
      inactive: data.filter(pm => !pm.is_active).length,
      byType: {}
    }
