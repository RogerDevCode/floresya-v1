...
export const getAllOccasions = asyncHandler(async (req, res) => {
  ...
  const occasions = await occasionService.getAllOccasions(filters, includeDeactivated)
...
export const getOccasionById = asyncHandler(async (req, res) => {
  const occasionId = ValidatorService.validateId(req.params.id, 'occasionId')
  const includeDeactivated = req.user?.role === 'admin'
  const occasion = await occasionService.getOccasionById(occasionId, includeDeactivated)
...
export const getOccasionBySlug = asyncHandler(async (req, res) => {
  const includeDeactivated = req.user?.role === 'admin'
  const occasion = await occasionService.getOccasionBySlug(req.params.slug, includeDeactivated)
...
export const updateOccasion = asyncHandler(async (req, res) => {
  const occasionId = ValidatorService.validateId(req.params.id, 'occasionId')
  const occasion = await occasionService.updateOccasion(occasionId, req.body)
...
export const updateDisplayOrder = asyncHandler(async (req, res) => {
  ...
  const occasion = await occasionService.updateDisplayOrder(occasionId, order)
...
export const deleteOccasion = asyncHandler(async (req, res) => {
  const occasionId = ValidatorService.validateId(req.params.id, 'occasionId')
  const occasion = await occasionService.deleteOccasion(occasionId)
...
export const reactivateOccasion = asyncHandler(async (req, res) => {
  const occasionId = ValidatorService.validateId(req.params.id, 'occasionId')
  const occasion = await occasionService.reactivateOccasion(occasionId)
