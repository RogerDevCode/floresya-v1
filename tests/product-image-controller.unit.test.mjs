...
const mockImageProcessor = {
  processImage: createMockFn(() => Promise.resolve({
    success: true,
    fileHash: 'testhash',
    sizes: { thumb: Buffer.from(''), small: Buffer.from(''), medium: Buffer.from(''), large: Buffer.from('') }
  })),
  generateFilename: createMockFn(() => 'product_1_1_testhash.webp')
};

const mockSupabaseStorage = {
  uploadImageSizes: createMockFn(() => Promise.resolve({
...
async function runTests() {
  ...
  runTest('getProductImages should handle valid product ID', async () => {
    ...
    mockProductImageService.getProductImages.mock = createMockFn(() => Promise.resolve([{ id: 1, size: 'medium' }]));
  ...
  runTest('getProductImages should handle invalid product ID', async () => {
    ...
    if (!res.json.mock.calls || res.json.mock.calls.length === 0) throw new Error('Response not sent');
    const response = res.data || res.sentData;
    if (response && response.success !== false) throw new Error('Expected success: false');
  });
  ...
  runTest('getPrimaryImage should handle valid product ID', async () => {
    ...
    mockProductImageService.getPrimaryImage.mock = createMockFn(() => Promise.resolve({ id: 1, size: 'medium', url: 'test.jpg' }));
  ...
  runTest('createProductImages should handle JSON body (backward compatibility)', async () => {
    const req = createMockRequest({ 
      ...
      body: {
        image_index: 1,
        images: [
          { size: 'thumb', url: 'test.jpg', file_hash: 'hash123', mime_type: 'image/jpeg' }
        ],
        is_primary: true
      }
    });
    ...
    mockProductImageService.createProductImagesAtomic.mock = createMockFn(() => Promise.resolve([
      { id: 1, size: 'thumb', url: 'test.jpg', is_primary: false }
    ]));
    ...
    const response = res.data || res.sentData;
    if (response && response.success !== true) throw new Error('Expected success: true');
    if (response && (!response.data || response.data.length !== 1)) throw new Error('Expected 1 image');
  });
  ...
  runTest('deleteImagesByIndex should handle valid parameters', async () => {
    ...
    mockProductImageService.deleteImagesByIndex.mock = createMockFn(() => Promise.resolve([
      { id: 1, size: 'thumb' },
      { id: 2, size: 'small' },
      { id: 3, size: 'medium' },
      { id: 4, size: 'large' }
    ...
    const response = res.data || res.sentData;
    if (response && response.success !== true) throw new Error('Expected success: true');
    if (response && (!response.data || response.data.deleted_count !== 4)) throw new Error('Expected deleted count of 4');
  });
  ...
  runTest('setPrimaryImage should handle valid parameters', async () => {
    ...
    mockProductImageService.setPrimaryImage.mock = createMockFn(() => Promise.resolve({ id: 1, size: 'medium', is_primary: true }));
