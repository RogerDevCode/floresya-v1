...
function setupIntegrationElements() {
  ...
  const mockElements = {
    ...
    'product-carousel-order': new MockElement('input', {
    ...
    if (selector === '.image-slot') {
      ...
      return Array.from({ length: 5 }, (_, i) => {
        ...
        slot.classList.contains = cls => {
          return (
            cls === 'drag-over' || cls === 'dragging' || cls === 'has-image' || cls === 'is-primary'
          )
        }
        return slot
      })
    }
    if (selector === '.edit-product-link') {
      return []
    }
    if (selector === '.delete-product-link') {
      return []
    }
    return []
  }
...
runTest(
  'Complete product form workflow should work end-to-end',
  done => {
...
runTest(
  'Occasions loading should populate the form correctly',
  done => {
    setupIntegrationElements()
