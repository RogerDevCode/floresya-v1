...
runTest('should fetch orders from API and process the response', () => {
  ...
  async function _fetchOrdersFromAPI() {
    try {
  ...
  if (allOrders.length !== 2) {
    throw new Error(`Expected 2 orders, got ${allOrders.length}`)
  }
  if (allOrders[0].id !== 2) {
    throw new Error(`Expected most recent order to be #2, got #${allOrders[0].id}`)
  }
  if (allOrders[1].id !== 1) {
    throw new Error(`Expected older order to be #1, got #${allOrders[1].id}`)
  }
  if (allOrders[0].customer_name !== 'María González') {
    throw new Error(`Expected 'María González', got '${allOrders[0].customer_name}'`)
  }
  if (allOrders[1].customer_name !== 'Juan Pérez') {
    throw new Error(`Expected 'Juan Pérez', got '${allOrders[1].customer_name}'`)
  }

  // Verify that the data is properly formatted
  if (typeof allOrders[0].total_usd !== 'number') {
    throw new Error('Total USD should be a number')
  }
  if (typeof allOrders[0].total_ves !== 'number') {
    throw new Error('Total VES should be a number')
  }
...
runTest('should update order status via API', () => {
  ...
  const orderId = 1
  const _newStatus = 'verified'

  // Simulate changeOrderStatus function
  async function _changeOrderStatus(orderId, newStatus) {
    try {
      // Update via API
...
runTest('should apply multiple filters and process correctly', () => {
  ...
  if (filtered.length !== 1) {
    throw new Error(`Expected 1 order after all filters, got ${filtered.length}`)
  }
  if (filtered[0].id !== 1) {
    throw new Error(`Expected order #1, got #${filtered[0].id}`)
  }
  if (filtered[0].customer_name !== 'Juan Pérez') {
    throw new Error(`Expected 'Juan Pérez', got '${filtered[0].customer_name}'`)
  }
})
...
runTest('should render order details correctly', () => {
  ...
  if (!htmlContent.includes('Pedido #1')) {
    throw new Error('HTML should contain order ID')
  }
  if (!htmlContent.includes('Juan Pérez')) {
    throw new Error('HTML should contain customer name')
  }
  if (!htmlContent.includes('juan@example.com')) {
    throw new Error('HTML should contain customer email')
  }
  if (!htmlContent.includes('+1234567890')) {
    throw new Error('HTML should contain customer phone')
  }
  if (!htmlContent.includes('Ramos de Rosas Rojas')) {
    throw new Error('HTML should contain product name')
  }
  if (!htmlContent.includes('$100.00')) {
    throw new Error('HTML should contain total')
  }
  if (!htmlContent.includes('Pendiente')) {
    throw new Error('HTML should contain status label')
  }
})
...
runTest('should handle pagination correctly', () => {
  ...
  if (pageData.currentPage !== 1) {
    throw new Error(`Expected page 1, got ${pageData.currentPage}`)
  }
  if (pageData.totalPages !== 3) {
    throw new Error(`Expected 3 total pages, got ${pageData.totalPages}`)
  }
  if (pageData.orders.length !== 20) {
    throw new Error(`Expected 20 items on first page, got ${pageData.orders.length}`)
  }
  if (pageData.orders[0].id !== 1) {
    throw new Error(`Expected first item ID 1, got ${pageData.orders[0].id}`)
  }
  if (pageData.orders[19].id !== 20) {
    throw new Error(`Expected last item ID 20, got ${pageData.orders[19].id}`)
  }

  // Test second page
  pageData = getPageData(orders, 2, itemsPerPage)
  if (pageData.currentPage !== 2) {
    throw new Error(`Expected page 2, got ${pageData.currentPage}`)
  }
  if (pageData.orders.length !== 20) {
    throw new Error(`Expected 20 items on second page, got ${pageData.orders.length}`)
  }
  if (pageData.orders[0].id !== 21) {
    throw new Error(`Expected first item ID 21, got ${pageData.orders[0].id}`)
  }
  if (pageData.orders[19].id !== 40) {
    throw new Error(`Expected last item ID 40, got ${pageData.orders[19].id}`)
  }
  ...
  if (pageData.currentPage !== 3) {
    throw new Error(`Expected page 3, got ${pageData.currentPage}`)
  }
  if (pageData.orders.length !== 13) {
    throw new Error(`Expected 13 items on last page, got ${pageData.orders.length}`)
  }
  if (pageData.orders[0].id !== 41) {
    throw new Error(`Expected first item ID 41, got ${pageData.orders[0].id}`)
  }
  if (pageData.orders[12].id !== 53) {
    throw new Error(`Expected last item ID 53, got ${pageData.orders[12].id}`)
  }

  // Test invalid page
  pageData = getPageData(orders, 5, itemsPerPage)
  if (pageData.currentPage !== 1) {
    throw new Error(`Invalid page should default to 1, got ${pageData.currentPage}`)
  }
  if (pageData.orders.length !== 20) {
    throw new Error(`Expected 20 items for invalid page, got ${pageData.orders.length}`)
  }
})
...
runTest('should update statistics based on filtered orders', () => {
  ...
  if (stats.pending !== 2) {
    throw new Error(`Expected 2 pending orders, got ${stats.pending}`)
  } // IDs 1, 7
  if (stats.processing !== 3) {
    throw new Error(`Expected 3 processing orders, got ${stats.processing}`)
  } // IDs 2 (verified), 3 (preparing), 4 (shipped)
  if (stats.completed !== 2) {
    throw new Error(`Expected 2 completed orders, got ${stats.completed}`)
  } // IDs 5, 8
  if (stats.cancelled !== 1) {
    throw new Error(`Expected 1 cancelled order, got ${stats.cancelled}`)
  } // ID 6
})
...
runTest('should enable/disable pagination buttons correctly', () => {
  ...
  if (!state.btnFirst.disabled) {
    throw new Error('First button should be disabled on first page')
  }
  if (!state.btnPrev.disabled) {
    throw new Error('Prev button should be disabled on first page')
  }
  if (state.btnNext.disabled) {
    throw new Error('Next button should be enabled on first page of multiple')
  }
  if (state.btnLast.disabled) {
    throw new Error('Last button should be enabled on first page of multiple')
  }

  // Test on middle page
  state = updatePaginationUI(5, 3)
  if (state.btnFirst.disabled) {
    throw new Error('First button should be enabled on middle page')
  }
  if (state.btnPrev.disabled) {
    throw new Error('Prev button should be enabled on middle page')
  }
  if (state.btnNext.disabled) {
    throw new Error('Next button should be enabled on middle page')
  }
  if (state.btnLast.disabled) {
    throw new Error('Last button should be enabled on middle page')
  }

  // Test on last page
  state = updatePaginationUI(5, 5)
  if (state.btnFirst.disabled) {
    throw new Error('First button should be enabled on last page')
  }
  if (state.btnPrev.disabled) {
    throw new Error('Prev button should be enabled on last page')
  }
  if (!state.btnNext.disabled) {
    throw new Error('Next button should be disabled on last page')
  }
  if (!state.btnLast.disabled) {
    throw new Error('Last button should be disabled on last page')
  }

  // Test single page
  state = updatePaginationUI(1, 1)
  if (!state.btnFirst.disabled) {
    throw new Error('First button should be disabled on single page')
  }
  if (!state.btnPrev.disabled) {
    throw new Error('Prev button should be disabled on single page')
  }
  if (!state.btnNext.disabled) {
    throw new Error('Next button should be disabled on single page')
  }
  if (!state.btnLast.disabled) {
    throw new Error('Last button should be disabled on single page')
  }
})
