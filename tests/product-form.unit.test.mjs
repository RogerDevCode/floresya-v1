...
runTest('handleFileSelect should process a selected file', (done) => {
  const mockElements = setupMockElements();
...
runTest('handleProductFormSubmit should handle valid form submission', (done) => {
  const mockElements = setupMockElements();
  ...
  mockFormElement.querySelectorAll = (selector) => {
    if (selector === 'input[name="occasions"]:checked') return [];
    return [];
  };
  mockFormElement.querySelector = (selector) => {
    if (selector === 'input[name="occasions"]') return new MockElement('input');
    return null;
  };
  ...
  import('../public/pages/admin/dashboard.js')
    .then((dashboardModule) => {
      ...
      // Mock toast
      const toast = require('../public/js/components/toast.js').toast;
