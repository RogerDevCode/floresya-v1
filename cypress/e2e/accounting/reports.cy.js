...
describe('Accounting Module - Reports Dashboard', () => {
  ...
  context('Dashboard Overview', () => {
    ...
    it('should display sales amount in USD', () => {
      ...
      cy.get('[data-cy=total-sales-amount]').invoke('text').should('match', /\$\d+(\.\d{2})?/);
    });

    it('should display expenses amount in USD', () => {
      cy.get('[data-cy=total-expenses-amount]').should('contain', '$');
      cy.get('[data-cy=total-expenses-amount]').invoke('text').should('match', /\$\d+(\.\d{2})?/);
    });

    it('should calculate profit/loss correctly', () => {
      let sales, expenses;

      cy.get('[data-cy=total-sales-amount]').invoke('text').then((text) => {
        sales = parseFloat(text.replace('$', '').replace(',', ''));
      });

      cy.get('[data-cy=total-expenses-amount]').invoke('text').then((text) => {
        expenses = parseFloat(text.replace('$', '').replace(',', ''));
      });

      cy.get('[data-cy=profit-loss-amount]').invoke('text').then((text) => {
  ...
  context('Export Functionality', () => {
    ...
    it('should download CSV report', () => {
      ...
      cy.readFile(`${downloadsFolder}/reporte-contabilidad.csv`, { timeout: 15000 })
        .should('exist');
    });
    ...
    it('should download PDF report', () => {
      ...
      const downloadsFolder = Cypress.config('downloadsFolder');
      cy.readFile(`${downloadsFolder}/reporte-contabilidad.pdf`, { timeout: 15000 })
        .should('exist');
    });
  });
  ...
  context('Expenses by Category Breakdown', () => {
    ...
    it('should calculate percentages correctly', () => {
      ...
      cy.get('[data-cy=expenses-breakdown] tbody tr').each(($row) => {
