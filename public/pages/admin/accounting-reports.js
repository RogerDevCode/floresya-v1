...
async function generateReport() {
  ...
  try {
    ...
    const response = await api.get(`/api/admin/accounting/reports?start_date=${startDate}&end_date=${endDate}`);
...
function displayReport(data) {
  ...
  document.getElementById('total-sales').textContent = `$${parseFloat(summary.total_sales || 0).toFixed(2)}`;
  document.getElementById('total-orders').textContent = `${summary.total_orders || 0} órdenes`;

  document.getElementById('total-expenses').textContent = `$${parseFloat(summary.total_expenses || 0).toFixed(2)}`;
  document.getElementById('total-expense-count').textContent = `${summary.total_expense_count || 0} gastos`;
  ...
  netProfitEl.classList.remove('text-blue-600', 'text-red-600', 'dark:text-blue-400', 'dark:text-red-400');
  if (netProfit >= 0) {
  ...
  const profitMargin = summary.total_sales > 0 
    ? ((netProfit / summary.total_sales) * 100).toFixed(1)
    : 0;
...
function displaySalesBreakdown(dailySales) {
  ...
  tableBody.innerHTML = sortedSales.map(item => `
...
function displayExpensesBreakdown(expenseCategories) {
  ...
  const sortedExpenses = [...expenseCategories].sort((a, b) => 
    parseFloat(b.total_expenses || 0) - parseFloat(a.total_expenses || 0)
  );
